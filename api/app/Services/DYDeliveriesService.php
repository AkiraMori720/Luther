...
use Gobl\ORM\ORM;
use DELIVERY\App\Db\DYDeliveriesController;
use DELIVERY\App\Db\DYDelivery;
...
/**
 * Class DYDeliveriesService
 *
 * to add item to deliveries
 * - POST    /deliveries
 *
 * to update property(ies) of the item with the given :delivery_id
 * - PATCH     /deliveries/:delivery_id
 *
 * to update property(ies) of all items in `dy_deliveries`
 * - PATCH     /deliveries
 *
 * to delete item with the given :delivery_id
 * - DELETE  /deliveries/:delivery_id
 *
 * to delete all items in `dy_deliveries`
 * - DELETE  /deliveries
 *
 * to get the item with the given :delivery_id
 * - GET     /deliveries/:delivery_id
 *
 * to get all items in dy_deliveries
 * - GET     /deliveries
 *
 * to get relation for the item in `dy_deliveries` with the given :delivery_id
 * - GET     /deliveries/:delivery_id/relation
 */
final class DYDeliveriesService extends BaseService
{
	//========================================================
	...
	/**
	 * Creates a new entry in the table `dy_deliveries`
	 *
	...
	public function actionCreateEntity(ORMRequest $orm_request)
	{
		...
		$controller = new DYDeliveriesController();
	...
	/**
	 * Updates only one item in the table `dy_deliveries` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	public function actionUpdateOneItem(ORMRequest $orm_request)
	{
		...
		if ($entity instanceof DYDelivery) {
			$this->getResponseHolder()
				 ->setDone($controller->getCRUD()
									  ->getMessage())
				 ->setData(['item' => $entity]);
	...
	/**
	 * Updates all items in the table `dy_deliveries` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	/**
	 * Deletes only one item in the table `dy_deliveries` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	/**
	 * Deletes all items in the table `dy_deliveries` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	/**
	 * Gets only one item from the table `dy_deliveries` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	/**
	 * Gets all items from the table `dy_deliveries` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	public function actionGetAll(ORMRequest $orm_request)
	{
		...
		if ($collection) {
			$table      = ORM::getDatabase('DELIVERY\App\Db')
							 ->getTable(DYDelivery::TABLE_NAME);
	...
	{
		if (!$orm_request->getColumnFilters(DYDelivery::COL_ID)) {
			throw new NotFoundException();
		}
	...
	/**
	 * @param \DELIVERY\App\Db\DYDelivery        $entity
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 ...
	 * @return array
	 */
	private function listEntityRelations(DYDelivery $entity, ORMRequest $orm_request, &$total_records = null)
	{
	...
	/**
	 * @param \DELIVERY\App\Db\DYDelivery[]      $entities
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 * @param int                       &$total_records
	 *
	 * @throws \OZONE\OZ\Exceptions\NotFoundException
	 ...
	 * @return mixed
	 */
	private function getRelationItem(Relation $relation, DYDelivery $entity)
	{
		$filters          = [];
	...
	public static function registerRoutes(Router $router)
	{
		...
		$options = [
			'delivery_id'    => $is_number ? '[0-9]+' : '[^/]+',
			'relation' => Relation::NAME_PATTERN,
		];

		$router->post('/deliveries', function (RouteInfo $r) {
			...
			return $service->respond();
			   ...
			   ->get('/deliveries/{delivery_id}', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('delivery_id', $r->getArg('delivery_id'));
			   	$service = new self($context);
			   	$service->actionGetEntity($orm_request);

			   	return $service->respond();
			   }, $options)
			   ->get('/deliveries', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$service     = new self($context);
			   ...
			   ->get('/deliveries/{delivery_id}/{relation}', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('delivery_id', $r->getArg('delivery_id'));
			   ...
			   ->patch('/deliveries/{delivery_id}', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('delivery_id', $r->getArg('delivery_id'));
			   ...
			   ->patch('/deliveries', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$service     = new self($context);
			   ...
			   ->delete('/deliveries/{delivery_id}', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('delivery_id', $r->getArg('delivery_id'));
			   ...
			   ->delete('/deliveries', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('delivery_id', $r->getArg('delivery_id'));
