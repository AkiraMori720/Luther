...
use Gobl\ORM\ORM;
use DELIVERY\App\Db\DYCurrenciesController;
use DELIVERY\App\Db\DYCurrency;
...
/**
 * Class DYCurrenciesService
 *
 * to add item to currencies
 * - POST    /currencies
 *
 * to update property(ies) of the item with the given :currency_code
 * - PATCH     /currencies/:currency_code
 *
 * to update property(ies) of all items in `dy_currencies`
 * - PATCH     /currencies
 *
 * to delete item with the given :currency_code
 * - DELETE  /currencies/:currency_code
 *
 * to delete all items in `dy_currencies`
 * - DELETE  /currencies
 *
 * to get the item with the given :currency_code
 * - GET     /currencies/:currency_code
 *
 * to get all items in dy_currencies
 * - GET     /currencies
 *
 * to get relation for the item in `dy_currencies` with the given :currency_code
 * - GET     /currencies/:currency_code/relation
 */
final class DYCurrenciesService extends BaseService
{
	//========================================================
	...
	/**
	 * Creates a new entry in the table `dy_currencies`
	 *
	...
	public function actionCreateEntity(ORMRequest $orm_request)
	{
		...
		$controller = new DYCurrenciesController();
	...
	/**
	 * Updates only one item in the table `dy_currencies` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	public function actionUpdateOneItem(ORMRequest $orm_request)
	{
		...
		if ($entity instanceof DYCurrency) {
			$this->getResponseHolder()
				 ->setDone($controller->getCRUD()
									  ->getMessage())
				 ->setData(['item' => $entity]);
	...
	/**
	 * Updates all items in the table `dy_currencies` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	/**
	 * Deletes only one item in the table `dy_currencies` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	/**
	 * Deletes all items in the table `dy_currencies` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	/**
	 * Gets only one item from the table `dy_currencies` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	/**
	 * Gets all items from the table `dy_currencies` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	public function actionGetAll(ORMRequest $orm_request)
	{
		...
		if ($collection) {
			$table      = ORM::getDatabase('DELIVERY\App\Db')
							 ->getTable(DYCurrency::TABLE_NAME);
	...
	{
		if (!$orm_request->getColumnFilters(DYCurrency::COL_CODE)) {
			throw new NotFoundException();
		}
	...
	/**
	 * @param \DELIVERY\App\Db\DYCurrency        $entity
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 ...
	 * @return array
	 */
	private function listEntityRelations(DYCurrency $entity, ORMRequest $orm_request, &$total_records = null)
	{
	...
	/**
	 * @param \DELIVERY\App\Db\DYCurrency[]      $entities
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 * @param int                       &$total_records
	 *
	 * @throws \OZONE\OZ\Exceptions\NotFoundException
	 ...
	 * @return mixed
	 */
	private function getRelationItem(Relation $relation, DYCurrency $entity)
	{
		$filters          = [];
	...
	public static function registerRoutes(Router $router)
	{
		...
		$options = [
			'currency_code'    => $is_number ? '[0-9]+' : '[^/]+',
			'relation' => Relation::NAME_PATTERN,
		];

		$router->post('/currencies', function (RouteInfo $r) {
			...
			return $service->respond();
			   ...
			   ->get('/currencies/{currency_code}', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('currency_code', $r->getArg('currency_code'));
			   	$service = new self($context);
			   	$service->actionGetEntity($orm_request);

			   	return $service->respond();
			   }, $options)
			   ->get('/currencies', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$service     = new self($context);
			   ...
			   ->get('/currencies/{currency_code}/{relation}', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('currency_code', $r->getArg('currency_code'));
			   ...
			   ->patch('/currencies/{currency_code}', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('currency_code', $r->getArg('currency_code'));
			   ...
			   ->patch('/currencies', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$service     = new self($context);
			   ...
			   ->delete('/currencies/{currency_code}', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('currency_code', $r->getArg('currency_code'));
			   ...
			   ->delete('/currencies', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('currency_code', $r->getArg('currency_code'));
