...
use Gobl\ORM\ORM;
use DELIVERY\App\Db\DYDeliverersController;
use DELIVERY\App\Db\DYDeliverer;
...
/**
 * Class DYDeliverersService
 *
 * to add item to deliverers
 * - POST    /deliverers
 *
 * to update property(ies) of the item with the given :deliverer_id
 * - PATCH     /deliverers/:deliverer_id
 *
 * to update property(ies) of all items in `dy_deliverers`
 * - PATCH     /deliverers
 *
 * to delete item with the given :deliverer_id
 * - DELETE  /deliverers/:deliverer_id
 *
 * to delete all items in `dy_deliverers`
 * - DELETE  /deliverers
 *
 * to get the item with the given :deliverer_id
 * - GET     /deliverers/:deliverer_id
 *
 * to get all items in dy_deliverers
 * - GET     /deliverers
 *
 * to get relation for the item in `dy_deliverers` with the given :deliverer_id
 * - GET     /deliverers/:deliverer_id/relation
 */
final class DYDeliverersService extends BaseService
{
	//========================================================
	...
	/**
	 * Creates a new entry in the table `dy_deliverers`
	 *
	...
	public function actionCreateEntity(ORMRequest $orm_request)
	{
		...
		$controller = new DYDeliverersController();
	...
	/**
	 * Updates only one item in the table `dy_deliverers` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	public function actionUpdateOneItem(ORMRequest $orm_request)
	{
		...
		if ($entity instanceof DYDeliverer) {
			$this->getResponseHolder()
				 ->setDone($controller->getCRUD()
									  ->getMessage())
				 ->setData(['item' => $entity]);
	...
	/**
	 * Updates all items in the table `dy_deliverers` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	/**
	 * Deletes only one item in the table `dy_deliverers` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	/**
	 * Deletes all items in the table `dy_deliverers` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	/**
	 * Gets only one item from the table `dy_deliverers` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	/**
	 * Gets all items from the table `dy_deliverers` that matches some filters
	 *
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 *
	 * @throws \Throwable
	...
	public function actionGetAll(ORMRequest $orm_request)
	{
		...
		if ($collection) {
			$table      = ORM::getDatabase('DELIVERY\App\Db')
							 ->getTable(DYDeliverer::TABLE_NAME);
	...
	{
		if (!$orm_request->getColumnFilters(DYDeliverer::COL_ID)) {
			throw new NotFoundException();
		}
	...
	/**
	 * @param \DELIVERY\App\Db\DYDeliverer        $entity
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 ...
	 * @return array
	 */
	private function listEntityRelations(DYDeliverer $entity, ORMRequest $orm_request, &$total_records = null)
	{
	...
	/**
	 * @param \DELIVERY\App\Db\DYDeliverer[]      $entities
	 * @param \OZONE\OZ\Core\ORMRequest $orm_request
	 * @param int                       &$total_records
	 *
	 * @throws \OZONE\OZ\Exceptions\NotFoundException
	 ...
	 * @return mixed
	 */
	private function getRelationItem(Relation $relation, DYDeliverer $entity)
	{
		$filters          = [];
	...
	public static function registerRoutes(Router $router)
	{
		...
		$options = [
			'deliverer_id'    => $is_number ? '[0-9]+' : '[^/]+',
			'relation' => Relation::NAME_PATTERN,
		];

		$router->post('/deliverers', function (RouteInfo $r) {
			...
			return $service->respond();
			   ...
			   ->get('/deliverers/{deliverer_id}', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('deliverer_id', $r->getArg('deliverer_id'));
			   	$service = new self($context);
			   	$service->actionGetEntity($orm_request);

			   	return $service->respond();
			   }, $options)
			   ->get('/deliverers', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$service     = new self($context);
			   ...
			   ->get('/deliverers/{deliverer_id}/{relation}', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('deliverer_id', $r->getArg('deliverer_id'));
			   ...
			   ->patch('/deliverers/{deliverer_id}', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('deliverer_id', $r->getArg('deliverer_id'));
			   ...
			   ->patch('/deliverers', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$service     = new self($context);
			   ...
			   ->delete('/deliverers/{deliverer_id}', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('deliverer_id', $r->getArg('deliverer_id'));
			   ...
			   ->delete('/deliverers', function (RouteInfo $r) {
			   	$context     = $r->getContext();
			   	$orm_request = new ORMRequest($context, $context->getRequest()
																   ->getFormData());
			   	$orm_request->addColumnFilter('deliverer_id', $r->getArg('deliverer_id'));
